...
def get_dataset(data_name, img_size, seq_len, temp_res_sevir,debug, **kwargs):
    ...
    if dataset_name == 'cikm':
    ...
    elif data_name == 'shanghai' or data_name=='shanghai_30mins':
        ...
        train = KNMITorchDataset(dataset_dir=train_path,
                                 ...
                                 debug=debug)
        val =  KNMITorchDataset(dataset_dir=val_path,
                                 img_size=img_size,
                                 seq_len=seq_len,
                                 batch_size=batch_size,
                                 debug=debug)
        test = KNMITorchDataset(dataset_dir=test_path,
                                 img_size=img_size,
                                 seq_len=seq_len,
                                 batch_size=batch_size,
                                 debug=debug)
        loader = train.get_torch_dataloader(num_workers=4)
        ...
        downsample_factor = int(temp_res_sevir/5)
        downsample_dict = {"vil":(downsample_factor,1,1)}
        print("downsample dict is ",downsample_dict)
        ...
        batch_size = kwargs.get('batch_size', 1)
        print("dataset dir is ", DATAPATH[data_name])
